...
# Pipeline RAG：检索结果由 Python 代码注入 prompt，此处说明如何使用注入内容
INTERVIEWER_RAG_ADDON = """

当 prompt 中出现【题库参考】时，表示系统已从题库检索到相关面试题。
请参考其中的考察要点和追问方向，结合候选人简历背景生成个性化问题。
不要原文照搬题库中的问题——改写使其更贴近候选人的具体经历。
若题库参考内容与候选人背景不符，忽略它，自主出题。"""
...
第二步：按三个维度分别打分（0-10）
  评分锚点（严格参照顶级科技公司面试标准）：
    9-10 → 极少见；回答堪称教科书级别，有深度洞见或超出预期的细节
    7-8  → 良好；核心要点清晰，有具体例子/数据，但缺乏深度或有小漏洞
    5-6  → 及格；能答出主要思路，但泛化/笼统，缺乏实质细节
    3-4  → 较差；回答模糊、偏题，或只说了表面概念
    0-2  → 无效；未回答问题，内容空洞或完全错误
  默认从 5 分开始，有实质内容才加分，有明显缺陷则减分。切勿因"态度好"或"方向对"就给 7+。

  content_score:   内容质量
    technical  → 技术准确性 + 实现细节深度（只说概念不说实现扣 2 分）
    behavioral → 事例真实性 + 结果量化程度（无量化结果上限 6 分）
    situational→ 方案合理性 + 考虑维度全面性
  structure_score: 表达结构
    technical  → 逻辑条理（问题→方案→权衡）
    behavioral → STAR 完整度（缺任何一要素扣 2 分）
